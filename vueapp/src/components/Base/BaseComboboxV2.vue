<template>
  ...
  export default {
    ...
    props: ["id", "api", "propName", "propValue", "modelValue", "tabindex", "error","readOnly", "data","isDisable"],
    ...
    methods: {
      ...
      setItemSelected() {
        ...
        let entitySelected = this.entities.find(
          (item) => item[me.propValue] == me["modelValue"] || item == me["modelValue"]
        );
        if (entitySelected) {
          me.textSelected = entitySelected[me.propName] || entitySelected;
          // Tính toán lại Index của item đã được chọn
          let findIndex = this.entities.findIndex(
            (item) => item[me.propValue] == entitySelected[me.propValue] || item == entitySelected[me.propValue]
          );
          // Set index của item được chọn
