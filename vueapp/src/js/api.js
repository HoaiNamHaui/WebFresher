var MISAapi = {
    "employee":{
        // newEmployeeCode : "https://cukcuk.manhnv.net/api/v1/Employees/NewEmployeeCode",
        // employeeApi: "https://cukcuk.manhnv.net/api/v1/Employees/",
        // filter: "https://cukcuk.manhnv.net/api/v1/Employees/filter?",
        newEmployeeCode : "https://localhost:7288/api/v1/Employees/NewEmployeeCode",
        employeeApi: "https://localhost:7288/api/v1/Employees/",
        filter: "https://localhost:7288/api/v1/Employees/EmployeeFilter?"
    },
    "department":{
        // departmentApi: "https://cukcuk.manhnv.net/api/v1/Departments"
        departmentApi: "https://localhost:7288/api/v1/Departments"
    }
}
export default MISAapi;